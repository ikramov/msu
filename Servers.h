...
{
public:
	session(boost::asio::io_context& io_context, int thr_num)
		: socket_(io_context), thread_number(thr_num)
	{	};
...
private:
	...
	boost::asio::io_context* io_contexts;
...
public:
	...
	~cond_var() { };
	void set_data(boost::asio::io_context& iocont, server *serv)
	{
		serverlink = serv;
		iocontext = &(iocont);
	}
